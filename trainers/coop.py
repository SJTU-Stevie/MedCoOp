...
class MedTextEncoder(nn.Module):
    def __init__(self, clip_model):
        super().__init__()
        self.encoder = clip_model.text_model.model.encoder
        self.pooler = clip_model.text_model.model.pooler
        self.projection = clip_model.text_model.projection_head

    def forward(self, prompts):
        encoder_output = self.encoder(prompts)
        output = self.pooler(encoder_output.last_hidden_state)
        label = self.projection(output)
        return label
...
class MedPromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        if ctx_init:
            ...
            prompt = clip_model.text_model.tokenizer(ctx_init)
            embedding = prompt
...
class CustomMedCLIP(nn.Module):
     def __init__(self, cfg, classnames, Medclip_model):
        super().__init__()
        self.prompt_learner = MedPromptLearner(cfg, classnames, Medclip_model)
        self.image_encoder = Medclip_model.vision_model
        self.text_encoder = MedTextEncoder(Medclip_model)
        self.logit_scale = Medclip_model.logit_scale
        self.dtype = Medclip_model.dtype
        self.compute_logits = Medclip_model.compute_logits
