    ...
    def __init__(self, checkpoint=None, medclip_checkpoint=None):
        ...
        if checkpoint is not None:
            state_dict = torch.load(os.path.join(
                checkpoint, constants.WEIGHTS_NAME))
...
class MedCLIPModel(nn.Module):
    def __init__(self,
        ...
        assert vision_cls in [
            MedCLIPVisionModel, MedCLIPVisionModelViT], 'vision_cls should be one of [MedCLIPVisionModel, MedCLIPVisionModelViT]'
    ...
    def from_pretrained(self, input_dir=None):
        ...
        if isinstance(self.vision_model, MedCLIPVisionModel):
            ...
            raise ValueError(
                f'We only have pretrained weight for MedCLIP-ViT or MedCLIP-ResNet, get {type(self.vision_model)} instead.')
        ...
        state_dict = torch.load(os.path.join(
            input_dir, constants.WEIGHTS_NAME))
        self.load_state_dict(state_dict)
...
class PromptClassifier(nn.Module):
    ...
    def forward(self, pixel_values=None, prompt_inputs=None, **kwargs):
        ...
        for cls_name, cls_text in prompt_inputs.items():
            ...
            if self.ensemble:
                # equivalent to prompt ensembling
                cls_sim = torch.mean(logits, 1)
            else:
...
class PartiallyFixedEmbedding(nn.Module):
    def __init__(self, fixed_weights, num_to_learn):
        ...
        weight = torch.empty(self.num_fixed+num_to_learn,
                             fixed_weights.size(1))
...
class PromptTuningClassifier(nn.Module):
    ...
    def __init__(self, medclip_model, n_context, class_specific_context, num_class, mode, ensemble=True,
        ...
        self.prev_embeddings = copy.deepcopy(
            self.model.text_model.model.embeddings.word_embeddings.weight.data)
        if not joint_train_emb:
            ...
            self.model.text_model.model.embeddings.word_embeddings = nn.Embedding(
                num_old + num_new, dim)
            self.model.text_model.model.embeddings.word_embeddings.weight.data[
                :num_old] = self.prev_embeddings
